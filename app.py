        ...
        with card():
            ...
            with tab_upload:
                ...
                if img_source:
                    ...
                    identify_busy = st.session_state.get("identify_in_progress", False)
                    if st.button(f"🔍 {t('btn_identify')}", key="identify_btn", disabled=identify_busy):
                        if not gm.is_configured():
                            st.error(t("err_gemini_not_configured"))
                        else:
                            st.session_state.identify_in_progress = True
                            status_slot = st.empty()
                            status_slot.markdown(
                                f'<div class="identify-status-badge">'
                                f'<span class="identify-spinner-dot"></span>{t("msg_looking")}</div>',
                                unsafe_allow_html=True,
                            )
                            try:
                                ...
                                st.session_state.identify_in_progress = False
